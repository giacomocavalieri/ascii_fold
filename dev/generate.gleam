...
fn loop(lines: List(String), acc: String) -> String {
  case lines {
    [] -> acc
    ["case " <> _, ..] -> case_loop(lines, [], acc)
    _ -> {
      panic
    }
  }
...
fn case_loop(lines: List(String), chars: List(String), code: String) {
  case lines {
    [] -> panic as "invalid code"
    ["case " <> _ as line, ..rest] ->
      case_loop(rest, [char_from_line(line), ..chars], code)
    [_, ..] -> output_loop(lines, chars, "", code)
  }
}
...
fn output_loop(
  ...
  case lines {
    ["    output[outputPos++] = '\"';", ..rest] ->
      output_loop(rest, chars, equivalent <> "\\\"", code)
    ["    output[outputPos++] = ''';", ..rest] ->
      output_loop(rest, chars, equivalent <> "'", code)
    ["    output[outputPos++] = '" <> char, ..rest] -> {
      let assert [char, _] = string.split(char, on: "'")
      output_loop(rest, chars, equivalent <> char, code)
    }
    ["    break;", ..rest] ->
      list.map(chars, fn(char) { "\"" <> char <> "\"" })
      |> string.join(with: " | ")
      |> string.append(" -> \"" <> equivalent <> "\"")
      |> string.append(code, _)
      |> loop(rest, _)
...
fn char_from_line(line) {
  let assert [_, after] = string.split(line, on: "// ")
  let assert Ok(#(char, _)) = string.split_once(after, on: " ")
  char
}

const code = "case '\\u00C0': // À  [LATIN CAPITAL LETTER A WITH GRAVE]
